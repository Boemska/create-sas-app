...
class adapterService {
	...
	_handleRequest(dispatch, promise, program, requestName) {

		if (requestName) program = requestName;
		//This get rid of the serach parameters
		else if (program.includes('?')) program = program.split('?')[0];
	 ...
	 */
	managedRequest(dispatch, method, url, options, requestName = null) {
		...
		this._handleRequest(dispatch, promise, url, requestName)
